...
export default function Shop() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      console.warn('Failed to load shop products');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen max-w-7xl mx-auto px-4 py-8 text-ivory">
      <Toast message={toastMsg} onClose={() => setToastMsg('')} />

      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-serif font-bold text-gold">Jewellery Catalog</h1>
        <p className="text-xs text-ivory/60 mt-1">Browse fine handcrafted 22k gold, certified solitaires, and royal gem collections.</p>
      </div>
      ...
      <div className="bg-ink/80 border border-gold/20 rounded-2xl p-4 mb-8 space-y-4">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <form onSubmit={handleSearchSubmit} className="relative w-full md:w-80">
            ...
            />
            <button type="submit" className="absolute right-3 top-2.5 text-gold">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-2 scrollbar-none">
          <span className="text-xs text-ivory/50 flex items-center gap-1 mr-1">
            <Filter className="w-3.5 h-3.5 text-gold" /> Categories:
          </span>
          {categories.map((cat) => (
            <button
              key={cat}
      ...
      <div className="flex justify-between items-center text-xs text-ivory/60 mb-4">
        <span>Showing {products.length} exquisite pieces</span>
        {selectedCategory !== 'All' && <span className="text-gold font-medium">Category: {selectedCategory}</span>}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-ink/40 border border-gold/10 rounded-2xl p-8 space-y-4">
          ...
          <h3 className="text-xl font-serif text-gold">No Matching Pieces Found</h3>
          <p className="text-xs text-ivory/60 max-w-sm mx-auto">
            Try adjusting your search criteria or explore our featured solitaire collections.
          </p>
          <button
            onClick={handleResetFilters}
